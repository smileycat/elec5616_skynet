...
def create_dh_key():
    ...
    g = 2;
    private_key = 2 + random.randint(0,prime-3)
...
def calculate_dh_secret(their_public, my_private):
    ...
    # by the difference length between prime and shared secret
    shared_secret = str(pow(their_public, my_private, prime))
    diff = len(str(prime)) - len(shared_secret)
    shared_secret = diff * '0' + shared_secret
